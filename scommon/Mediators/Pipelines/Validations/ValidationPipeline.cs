...
public class ValidationPipeline : IPipeline
{
    ...
    {
        var requestValidator = _provider.GetService<IValidator<T>>();
        if (requestValidator != null)
        {
            var requestResult = await requestValidator.ValidateAsync(instance, ct).ConfigureAwait(false);
            if (!requestResult.IsValid)
            {
                var messageItems = requestResult.Errors.Select(v =>
                    new MessageItem
                    {
                        Table = v.ErrorMessage,
                        Property = v.PropertyName,
                        Code = v.ErrorCode
                    }).ToList();
                
                throw new RequestValidationException(messageItems);
            }
        }

        var validator = _provider.GetService<IValidationHandler<T>>();
        if (validator == null)
        {
            return;
        }
        ...
        throw new ValidationException(result.Messages.FirstOrDefault()!);
    }
}
